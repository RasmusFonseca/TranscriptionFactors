...
import {Flaremodel} from "./flareplot/flaremodel.js";
...
export class TissueViewer {
  ...
  constructor(containerSelector, fingerprintSelector, width, height){
    this.width = width;
    this.height = height;
    const that = this;
    this.setupFlareplot(containerSelector)
      .then(function(){
          console.log("Setting up forceplot");
        that.setupForceplot(containerSelector);
          console.log("Setting up fingerprint panel");
  ...
  setupFlareplot(containerSelector) {
    ...
    return new Promise(resolve => {
      d3.json("Extended_Data_Table_4.json")
        .then(function (data) {
            console.log("Data read");
          that.flaremodel = new Flaremodel(data);
          //that.flareplot = new Flareplot(data, that.width, "#flareplotDiv");
          //that.flaremodel = that.flareplot.getModel();
            console.log("Model created");
          that.flaremodel.setFrames({ type: 'range', begin: 0, end: 110 });
